...
type LocalSIDDescriptor struct {
	...
	ValueComparator      func(key string, oldValue, newValue *linux_srv6.LocalSID) bool
	NBKeyPrefix          string
	WithMetadata         bool
	MetadataMapFactory   MetadataMapFactory
	Validate             func(key string, value *linux_srv6.LocalSID) error
	Create               func(key string, value *linux_srv6.LocalSID) (metadata interface{}, err error)
	Delete               func(key string, value *linux_srv6.LocalSID, metadata interface{}) error
	Update               func(key string, oldValue, newValue *linux_srv6.LocalSID, oldMetadata interface{}) (newMetadata interface{}, err error)
	UpdateWithRecreate   func(key string, oldValue, newValue *linux_srv6.LocalSID, metadata interface{}) bool
	Retrieve             func(correlate []LocalSIDKVWithMetadata) ([]LocalSIDKVWithMetadata, error)
	IsRetriableFailure   func(err error) bool
	DerivedValues        func(key string, value *linux_srv6.LocalSID) []KeyValuePair
	Dependencies         func(key string, value *linux_srv6.LocalSID) []Dependency
...
type LocalSIDDescriptorAdapter struct {
	descriptor *LocalSIDDescriptor
}

func NewLocalSIDDescriptor(typedDescriptor *LocalSIDDescriptor) *KVDescriptor {
	adapter := &LocalSIDDescriptorAdapter{descriptor: typedDescriptor}
...
func (da *LocalSIDDescriptorAdapter) ValueComparator(key string, oldValue, newValue proto.Message) bool {
	typedOldValue, err1 := castLocalSIDValue(key, oldValue)
	typedNewValue, err2 := castLocalSIDValue(key, newValue)
...
func (da *LocalSIDDescriptorAdapter) Validate(key string, value proto.Message) (err error) {
	typedValue, err := castLocalSIDValue(key, value)
	if err != nil {
		return err
	}
...
func (da *LocalSIDDescriptorAdapter) Update(key string, oldValue, newValue proto.Message, oldMetadata Metadata) (newMetadata Metadata, err error) {
	oldTypedValue, err := castLocalSIDValue(key, oldValue)
	if err != nil {
		return nil, err
	}
	newTypedValue, err := castLocalSIDValue(key, newValue)
	if err != nil {
		return nil, err
	}
	typedOldMetadata, err := castLocalSIDMetadata(key, oldMetadata)
	if err != nil {
		return nil, err
	}
...
func (da *LocalSIDDescriptorAdapter) Delete(key string, value proto.Message, metadata Metadata) error {
	...
	typedMetadata, err := castLocalSIDMetadata(key, metadata)
	if err != nil {
		return err
	}
...
func (da *LocalSIDDescriptorAdapter) Retrieve(correlate []KVWithMetadata) ([]KVWithMetadata, error) {
	var correlateWithType []LocalSIDKVWithMetadata
	for _, kvpair := range correlate {
		typedValue, err := castLocalSIDValue(kvpair.Key, kvpair.Value)
		if err != nil {
			continue
		}
		typedMetadata, err := castLocalSIDMetadata(kvpair.Key, kvpair.Metadata)
		if err != nil {
			continue
		}
		correlateWithType = append(correlateWithType,
			LocalSIDKVWithMetadata{
				Key:      kvpair.Key,
				Value:    typedValue,
...
func castLocalSIDValue(key string, value proto.Message) (*linux_srv6.LocalSID, error) {
	typedValue, ok := value.(*linux_srv6.LocalSID)
	if !ok {
...
func castLocalSIDMetadata(key string, metadata Metadata) (interface{}, error) {
	if metadata == nil {
		return nil, nil
	}
